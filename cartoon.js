...
const no_line = d3.line()
    .x(function(d) { return sc_x(d.t); })
    .y(function(d) { return sc_y(d.y); });

const yes_line = d3.line()
    .x(function(d) { return sc_x(d.t); })
    .y(function(d) { return sc_y(d.x); });

const doubt_line = d3.line()
	.x(function(d) { return sc_x(d.t); })
    .y(function(d) { return sc_y(d.z); });
...
const initialize = (controls,grid) => {
	...
	plot.append("path").datum(trajectory).attr("d",yes_line)
		.style("stroke",cfg.simulation.color.yes)
		.attr("id","yes")
		.attr("class",styles.curve)
		
	plot.append("path").datum(trajectory).attr("d",no_line)
		.style("stroke",cfg.simulation.color.no)
		.attr("id","no")
		.attr("class",styles.curve)

	plot.append("path").datum(trajectory).attr("d",doubt_line)
		.style("stroke",cfg.simulation.color.doubt)
		.attr("id","doubt")
		.attr("class",styles.curve)
...
const update = (controls) => {
	...
	controls.select("#yes").datum(trajectory).attr("d",yes_line)
	controls.select("#no").datum(trajectory).attr("d",no_line)	
	controls.select("#doubt").datum(trajectory).attr("d",doubt_line)	
}
